...
Lake <- read.csv(here("./Data/Processed_KEY/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv"), 
...
my_theme <- theme_classic() + 
  theme(axis.title = element_text("black"), #made axis titles black
        plot.background = element_rect("gray"), #made plot background gray
        axis.text = element_text("black"), #made axis text black
...
LakePlot <- ggplot(Lake, aes(x = po4, y = tp_ug, color = lakename)) + #created plot for total phosphorus and phosphate
  geom_point(na.rm = TRUE) + #got rid of NAs 
  geom_smooth(method = "lm", se = FALSE, color = "black") + #added a best fit line and made it black
  xlim(c(0, 50)) + #set limits for x-axis
  ylim(c(0, 150)) + #set limits for y-axis
  labs(x = "Phosphate (ug)", y = "Total Phosphorus (ug)", title = "Total Phosphorus by Phosphate", color = "Lake Name") #changed labels
...
temp_plot <- ggplot(Lake, aes(x = month.abb[month], y = temperature_C, color = lakename)) +
  ...
  labs(x = "Month", y = "Temperature(C)", title = "Temperature by Month", color = "Lake Name") #changed labels
...
TP_plot <- ggplot(Lake, aes(x = month.abb[month], y = tp_ug, color = lakename)) +
  ...
  scale_x_discrete(limits = month.abb) + #made x-axis labels show every month in abbreviated form
  labs(x = "Month", y = "Total Phosphorus(ug)", title = "Total Phosphorus by Month", color = "Lake Name") #changed labels
...
TN_plot <- ggplot(Lake, aes(x = month.abb[month], y = tn_ug, color = lakename)) +
  ...
  scale_x_discrete(limits = month.abb) + ##made x-axis labels show every month in abbreviated form
  labs(x = "Month", y = "Total Nitrogen(ug)", title = "Total Nitrogen by Month", color = "Lake Name") #changed labels
...
plot_all <- plot_grid(
  ...
  TN_plot + theme(legend.position = "none"), #got rid of legend
  nrow = 3, align = 'hv', rel_heights = c(1,1,1)) + #set number of rows, aligned horizontally and vertically, and set relative heights of the plots
  theme(axis.text = element_text(size = 10)) #changed axis text to size 10

legend_all <- get_legend(plot_all) #extracted out the legends

plot_all <- plot_grid(plot_all,
                      legend_all, rel_widths = c(1,0.25)
) #added legend to the cowplot
...
Needles1 <- ggplot(subset(Litter, functionalGroup == "Needles"), 
  ...
  labs(x = "Date", y = "Dry Mass", title = "Dry Mass of Needles by Date", color = "NLCD Class") #changed labels 
...
Needles2 <- ggplot(subset(Litter, functionalGroup == "Needles"), aes(x = collectDate, y = dryMass)) +
  ...
  labs(x = "Date", y = "Dry Mass", title = "Dry Mass of Needles by Date") #changed labels and title
