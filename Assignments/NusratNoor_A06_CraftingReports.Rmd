...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "Monthly Ozone (2018)", fig.align='left', fig.dim= c(5,4)}
ozone_2018 <- filter(epa_data, Year == 2018)

ozone_2018_box <- ggplot(ozone_2018, aes(x = month.abb[Month], y = Ozone)) +
  geom_boxplot() +
  scale_x_discrete(limits = month.abb) +
  labs(x = "Month", y = expression(paste("Concentration (", mu,"g/", m^{3}, ")")), title = "Recorded Ozone by Month (2018)")
print(ozone_2018_box)  
```

```{r echo=FALSE, message=FALSE, warning=FALSE, fig.cap= "Monthly Ozone (2019)", fig.align='left', fig.dim= c(5,4)}
ozone_2019 <- filter(epa_data, Year == 2019)

ozone_2019_box <- ggplot(ozone_2019, aes(x = month.abb[Month], y = Ozone)) +
  geom_boxplot() +
  scale_x_discrete(limits = month.abb) +
  labs(x = "Month", y = expression(paste("Concentration (", mu,"g/", m^{3}, ")")), title = "Recorded Ozone by Month (2019)")
print(ozone_2019_box) 
```
...
<https://github.com/nunoor/EDE_Fall2023>
