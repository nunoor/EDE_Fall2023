...
```{r, message = FALSE}
#1
GaringerOzone_path <- list.files(here("Data/Raw/Ozone_TimeSeries/"), pattern = "\\.csv$", full.names = TRUE)
...
# 4
GaringerOzone_wr <- select(GaringerOzone, "Date", "Daily.Max.8.hour.Ozone.Concentration", "DAILY_AQI_VALUE")
#wrangled data to only include the asked for columns

# 5
Days <- as.data.frame(seq(from = as.Date("2010-01-01"), to = as.Date("2019-12-31"), by = "day")) #created new data frame of dates
...
ggplot(GaringerOzone, 
  ...
  labs(x = "Date", y = "Ozone Concentration (ppm)", title = "Ozone Concentration Over Time for Garinger High School")
#created line plot of ozone concentration by date and added linear model trend line and cleaned up labels
```
...
GaringerOzone.monthly <- GaringerOzone %>% #created new monthly data frame
  ...
  summarise(mean_ozone = mean(Daily.Max.8.hour.Ozone.Concentration)) %>% #summarised the mean ozone concentration for each month
...
#decomposed the monthly time series and plotted it
```
...
#created graph of mean monthly ozone concentration over time with both a point and line layer and changed labels and added linear model trend line
```
...
```{r}
#15
GaringerOzone.monthly.componants <- as.data.frame(GaringerOzone.monthly.decom$time.series[,1:3])
#created new data frame of the extracted components of the time.series column
GaringerOzone.monthly.componants <- mutate(GaringerOzone.monthly.componants,
                                           Observed = GaringerOzone.monthly$mean_ozone, Date = GaringerOzone.monthly$Date2, noseason = Observed - seasonal)
#created 3 more columns of the Observed, Date, and noseason
GO.monthly.comp.ts <- ts(GaringerOzone.monthly.componants$noseason, start = c(2010,1), end = c(2019,12), frequency = 12)
