...
```{r DataExercise-1a}
high_temp <- c(81, 71, 72, 63, 51, 52, 61, 64, 67, 73)
...
```{r DataExercise-1c}
ten_day_forcast<- (data.frame(high_temp, high_temp_C, low_temp, low_temp_C))
...
#you can also do this in one step i.e. giving it a better name and making a data frame at the same time
```
...
#cannot use sd on a data frame, use it for an element in your data frame

sd(high_temp) #sd = standard deviation
