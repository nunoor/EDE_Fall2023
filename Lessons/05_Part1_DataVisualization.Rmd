...
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point()
#if i assign color in the geomplot() then it means i color the points ot lines or box and I specify the color itself "blue" or "green". but if i assign a color inside the ggplot aes, then that means i want it to color something based off a specific facttor/colname i.e. color coding my plot. 
...
#print(O3plot2)
#doing this ends up making it pink bc by adding the color to the aesthetics, you just added a blue layer on top of the black and didnt replace it
O3plot2 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2) #now its blue bc its now outside of the aestheics parenthesis
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? because fill is a function that changes the color of the inside of the boxes in the box plot and not the contour. but using color, you canche the contour i.e. the outline of the box plot, including the dots
